...
quest'ultimi, vengono supportati presso il CNAF anche gli esperimenti non-LHC
...
\begin{itemize}
    \item \textit{categorico nominale}, se contiene valori a scelta in un
        insieme finito;
...
\begin{definition}
    Un \textit{guasto} è un comportamento anomalo a livello software o
    hardware, che può causare stati illeciti (\textit{errori}) nel sistema o
    nell'applicazione e che, nel peggiore dei casi, può causare l'interruzione
    dell'applicazione o del sistema (\textit{fallimenti}).
\end{definition}
